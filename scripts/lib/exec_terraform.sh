...
# cp "./output_config.yaml" "/tf/avm/terraform/modules/landingzone/configuration/0-launchpad/scripts/config.yaml"
...
working_path="/tf/avm/terraform/modules/landingzone/configuration/1-landingzones/application/networking_spoke_project"
exec_terraform $backend_config_key $working_path $RG_NAME $STG_NAME "1-landingzones"

# spoke devops 
backend_config_key="network-spoke-devops"
working_path="/tf/avm/terraform/modules/landingzone/configuration/1-landingzones/application/networking_spoke_devops"
exec_terraform $backend_config_key $working_path $RG_NAME $STG_NAME "1-landingzones" 

# peering project-devops
backend_config_key="network-peering-project-devops"
working_path="/tf/avm/terraform/modules/landingzone/configuration/1-landingzones/application/networking_peering_project_devops"
exec_terraform $backend_config_key $working_path $RG_NAME $STG_NAME "1-landingzones" 
...
for section in $(yq 'keys | .[]' "$yaml_file" -r); do
  ...
  for key in $(yq ".${section} | keys | .[]" "$yaml_file" -r); do
    ...
    if [ $value = "true" ]; then
      ...
      working_path="/tf/avm/terraform/modules/landingzone/configuration/2-solution_accelerators/${section}/${key}"
