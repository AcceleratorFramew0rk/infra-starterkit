...
# USAGE:
# cd /tf/avm
# ./scripts/bin/import.sh
# OR
# ./scripts/bin/import.sh /tf/avm 3
#------------------------------------------------------------------------


# If $1 is unset or empty, default to /tf/avm
WORKING_DIR="${1:-/tf/avm}"
COMPARTMENT_TYPE="${2:-3}"
echo "Working Directory: ${WORKING_DIR}"
echo "Compartment Type: ${COMPARTMENT_TYPE}"
...
CWD=$(pwd)
echo $CWD
CONFIG_FILE_PATH="${WORKING_DIR}/config/config.yaml"
...
# go to the launchpad folder
echo "go to the launchpad folder <$WORKING_DIR/templates/landingzone/configuration/0-launchpad/launchpad>"  
...
VNET_RESOURCE_GROUP_NAME=$(yq -r '.resource_group_name' $CONFIG_FILE_PATH)
